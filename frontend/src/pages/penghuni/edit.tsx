...
function EditPenghuni({
  ...
  const [fotoKTP, setFotoKTP] = useState<File | string>("");
  ...
  useEffect(() => {
    if (data) {
      ...
      form.setValue("menikah", data.menikah);
      form.setValue("noTelepon", data.noTelepon);
      setFotoKTP(imageFromBackend(data.fotoKTP));
    }
  }, [data, form]);

  const penghuniMutation = useMutation({
    mutationKey: ["edit-penghuni"],
    mutationFn: async (value: FormData) => {
      return (
        await axiosInstance.put(`/penghuni/${data.id}`, value, {
          headers: {
    ...
    onSuccess: (payload) => {
      toast.success(`Penghuni ${payload.nama} berhasil diedit`);
      setIsOpen(false);
      queryClient.invalidateQueries({ queryKey: ["penghuni"] });
    },
    onError: () => {
      toast.error("Penghuni gagal diedit");
    },
  });
            ...
            >
              ...
              <div>
                ...
                {fotoKTP ? (
                  <img
                    src={
                      fotoKTP instanceof File
                        ? URL.createObjectURL(fotoKTP)
                        : typeof fotoKTP === "string"
                        ? fotoKTP
                        : ""
                    }
                    width={224}
